...
generate_group_rep k =
  fq (k+1) >>= \(p,q,pq) -> sl2_fq_rep_sym (p,q,pq) ("u","t","h2","h") >>= \sl2_rep ->
  return (pq,sl2_rep)

obfuscate_group k (pq,rep) =
  random_tietze rep (\rep -> sample_from_rep_2 k rep pq) k
...
main =
  let k = 10 in
  generate_group_rep k >>= \(pq,sl2_rep) ->
  obfuscate_group k (pq,sl2_rep) >>= \(sl2_rep_obfuscated,rev_trace) ->
  let phi = (calculate_value_from_rev_trace rev_trace sl2_rep_obfuscated) in
  let sample_G = sample_from_rep_2 k sl2_rep pq in
  let sample_K = sample_from_rep_2 k sl2_rep_obfuscated pq in
  sample_G >>= \v ->
  putStrLn . show $ (v , phi (fst v))
